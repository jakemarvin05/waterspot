...
<div>
	...
	?>
    	...
    	<?php foreach($booked_slots as $book_slot) {
    		...
    		$table_str .= '<td colspan="2">'.$book_slot['price'].'</td>';
    		$table_str .= '<td colspan="2">Booked over the phone.</td>';
			$table_str .= '<td>'.$this->Html->link($this->Html->image('del.png'),array('plugin'=>'vendor_manager','controller'=>'services','action'=>'slot_delete',$service_id,$book_slot['id']),array('escape'=>false,"onclick"=>"return confirm('Are you sure you wish to delete this slot?')")).'</td>';
    		$table_str .= '</tr>';
			$table .= $table_str;
		}
		?>
